...
export default function Stake() {
  ...
  const { pool, loading, approve, stake, claim, withdraw } = usePool()
  ...
  const [stakeDisabled, stakeText] = useMemo(() => {
    ...
    if (pool!.allowance === 0) {
      ...
      disabled = true
      text = `Insufficient ${pool?.stake} Balance`
      ...
      <div className={classNames(styles.block, styles.blockWrapper)}>
        <div>
          <div className={styles.title}>
            Stake {pool?.stake} to earn {pool?.earn}
          </div>
          <div className={styles.mainTitle}>
            {pool?.balance?.toLocaleString()}
          </div>
          <div className={styles.subTitle}>Current {pool?.stake} Balance</div>
          <div className={styles.mainTitle}>{supplyInfo.apy.toFixed(2)}%</div>
